...
export default function Dashboard() {
      ...
      const [employees, departments, leaves_requested, employees_on_leave] =
        await Promise.all([
          ...
          pocketbase
            ...
            .then((e) => e.reduce((a, b) => a + b.amount, 0)),
          pocketbase
            .collection("leaves")
            .getList(1, 1, {
          ...
          pocketbase
            .collection("leaves")
            .getList(1, 1, {
              filter: `status="approved" && start >= "${formatDateToStartOfDay(
                new Date()
              )}" && end <= "${formatDateToEndOfDay(new Date())}"`,
            })
            .then((e) => e.totalItems),
        ]);
  ...
  const fetchAttendaceData = async () => {
    ...
    //   ? all_employee.totalItems - today_attendance.totalItems
    //   : 0;

    console.log(all_employee.totalItems, today_attendance.totalItems);

    const absent = all_employee.totalItems - today_attendance.totalItems;
